...
| `CODEX_HOME`, `NODE_REPL_TRUSTED_CODE_PATHS` | `nne` always supplies the selected Codex home and a Linux colon-joined trust list containing that home and the selected module directories | LCU inherits `CODEX_HOME` if present and prepends its bundled module/plugin roots to caller trust roots. It does not itself add the selected Codex home. The trust domains therefore differ. Automatic approval review rejected expanding this trust boundary; that production change remains unapplied and is a parity blocker. The upstream test baseline uses its own private fixture home exactly as original `nne` does. |
...
CODEX_HOME trust expansion is blocked by automatic approval review. This is an unresolved integration difference, not an absent Linux feature. The local IAB registry and original feature normalizer now consume the effective build flavor; final-archive tests remain necessary.
