...
Cual's short launcher supplies explicit module search and trusted-code paths. The unmodified REPL isolates its trusted service as designed. No app login, OpenAI API key, desktop-app process, Codex CLI, or app socket was required in the isolated standalone tests. This does not prove application-level sandbox isolation or broad application compatibility.
