...
Instructions are derived from pinned upstream files using the reviewed line edits in `scripts/instructions.lock.json`. The build preserves every line outside those edits and rejects source or output drift. The edits remove other platforms and disabled browser-provider APIs, name the standalone `js` tool, and correct Linux-specific signatures and the shipped app-name entrypoint. The upstream Linux full-desktop skill is also retained as a reference with its client bound to `cua.computer`. The confirmation policy is unchanged. See the [complete instruction audit](INSTRUCTIONS.md) for every source file, edit, exclusion, and verification boundary.
