...
A local IAB host is being integrated and tested separately in the current development work. Its eventual verification must exercise the actual provider and state its host dependencies. The package inventory and the configuration-mode matrix alone do not prove IAB operations.
