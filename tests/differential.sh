...
mkdir "$scratch/tests"
cp "$tests"/differential* "$tests"/host_delivery.* "$tests"/iab* "$tests/codex_lifecycle.py" "$tests/browser_runtime.py" "$tests/native_pipe.py" "$tests/mcp_client.py" "$tests/x11_fixture.py" "$scratch/tests/"
tests="$scratch/tests"
dpkg-deb --extract "$package" "$scratch/upstream"
tar -xzf "$archive" -C "$scratch"
bundle="$scratch/$(basename -- "$archive" .tar.gz)"
"$bundle/scripts/install.sh" --user "$(id -un)" --prefix "$scratch/installed" --runtime-only --skip-system --yes
...
python3 - "$DIFFERENTIAL_UPSTREAM_RUNTIME" "$scratch/installed/current/runtime" "$output/runtime-inventory.json" <<'PY'
...
for side in upstream lcu; do
  ...
  if [[ $side == upstream ]]; then
    ...
    export DIFFERENTIAL_RUNTIME="$scratch/installed/current/runtime"
  fi
...
dbus-run-session -- bash "$tests/iab.sh" "$scratch/installed/current" "$output/iab"
